...
export default {
    ...
    methods: {
        async save() {
            this.loading = true;
            await API.tagEndpoint.save(this.form)
                .then(response => {
        ...
        async getById() {
            this.loading = true;
            await API.tagEndpoint.getById(this.tagId)
                .then(response => {
                    if (response.data.status) {
                    ...
                    NotificationHelper.showError('Возникли ошиб при сохранении! Перезагрузите страницу и повторите попытку!')
                }).finally(() => {
                    this.loading = false;
                })
        }
